...
tokens { AFFECT; COND; IF; BLOCK; PLUS; MOINS;}
...
vardec			: 'var' ID vardec1 -> ^(AFFECT ID vardec1)
			;
...
letexp   		: 'let' dec+ 'in' exp (';' exp)* 'end' ->  dec+  exp+ 
			;
...
ifthenelse 		: 'if' exp 'then'  exp ifelse ->^(IF exp exp )
			;
...
whileexp 		: 'while' exp 'do' exp -> ^(WHILE exp exp)
			;
